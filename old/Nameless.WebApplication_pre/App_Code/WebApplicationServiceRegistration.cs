...
namespace Nameless.WebApplication {
    public sealed class WebApplicationServiceRegistration : ServiceRegistrationBase {
        ...
        private static void ConfigureAuth (IConfiguration config, IServiceCollection services) {
            ...
            var authConfig = config.GetSection (nameof (AuthConfiguration));
            services.ConfigureSettings (authConfig, () => new AuthConfiguration { SecretKey = "4bbdb924-4b34-482f-8cf6-5f49722c0652" });

            var signingKey = new SymmetricSecurityKey (Encoding.ASCII.GetBytes (authConfig[nameof (AuthConfiguration.SecretKey)]));

            // jwt wire up
            // Get options from app settings
            var jwtAppSettingOptions = config.GetSection (nameof (JwtIssuerOptions));
            // Configure JwtIssuerOptions
            services.ConfigureSettings (jwtAppSettingOptions, () => new JwtIssuerOptions {
            ...
            services
                ...
                .AddJwtBearer (opts => {
                    opts.ClaimsIssuer = jwtAppSettingOptions[nameof (JwtIssuerOptions.Issuer)];

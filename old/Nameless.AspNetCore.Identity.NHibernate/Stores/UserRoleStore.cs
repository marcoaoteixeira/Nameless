...
namespace Nameless.AspNetCore.Identity.Stores {
    public partial class UserStore : IUserRoleStore<User> {
        ...
        public Task<bool> IsInRoleAsync (User user, string roleName, CancellationToken cancellationToken) {
            ...
            var userInRoleCriteria = Session.CreateCriteria<UserRole> ();

            userInRoleCriteria
                .Add (Restrictions.Eq (nameof (UserRole.UserID), user.ID))
                .Add (Restrictions.Eq (nameof (UserRole.RoleID), role.ID));

            cancellationToken.ThrowIfCancellationRequested ();
            var isInRole = userInRoleCriteria.UniqueResult<UserRole> () != null;

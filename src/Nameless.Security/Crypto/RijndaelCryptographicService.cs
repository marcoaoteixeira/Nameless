...
public sealed class RijndaelCryptographicService : ICryptographicService, IDisposable {
    ...
    public RijndaelCryptographicService(IOptions<RijndaelCryptoOptions> options,
                                        ILogger<RijndaelCryptographicService> logger) {
    ...
    private ICryptoTransform GetEncryptor() {
        return _encryptor ?? throw new InvalidOperationException("Encryptor not available.");
    }

    private ICryptoTransform GetDecryptor() {
        return _decryptor ?? throw new InvalidOperationException("Decryptor not available.");
    }
    ...
    private byte[] AddSalt(byte[] plainTextBytes) {
        ...
        Array.Copy(plainTextBytes,
            0,
            plainTextBytesWithSalt,
            saltBytes.Length, plainTextBytes.Length);

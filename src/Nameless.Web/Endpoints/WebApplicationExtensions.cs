        ...
        private IEndpointDescriptor[] CreateEndpointDescriptors() {
            ...
            foreach (var endpointType in endpointTypeCollection) {
                if (serviceResolver.Create(endpointType) is not IEndpoint endpoint) {
                    // This probably will never happen because we filtered all
                    // valid implementations during registration, but let's cover
                    // it anyway.
                    throw new InvalidOperationException(
                        $"Unable to create instance of '{endpointType.FullName}'."
                    );
                }

                result.Add(endpoint.Describe());

                // We are interested only in the endpoint descriptor for the
                // moment. So it's safe to just dispose it, if needed.

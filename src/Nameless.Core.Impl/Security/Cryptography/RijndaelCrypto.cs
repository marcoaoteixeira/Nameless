...
public class RijndaelCrypto : ICrypto, IDisposable {
    ...
    private readonly ILogger<RijndaelCrypto> _logger;
    ...
    public RijndaelCrypto(IOptions<RijndaelCryptoOptions> options, ILogger<RijndaelCrypto> logger) {
    ...
    private byte[] CreateKeyBuffer() {
        ...
        var keySize = Enum.IsDefined(options.KeySize)
            ? (int)options.KeySize
            : (int)KeySize.Large;
        ...
        return Rfc2898DeriveBytes.Pbkdf2(
            ...
            salt: saltValueBytes,
            iterations: options.KeyIterationsCount,
            hashAlgorithm: HashAlgorithmName.SHA256,
            outputLength: keySize / DeriveSize
        );
    }
    ...
    private byte[] AddSalt(byte[] plainTextBytes) {
        ...
        Array.Copy(
            sourceArray: plainTextBytes,
            sourceIndex: 0,
            destinationArray: plainTextBytesWithSalt,
            destinationIndex: saltBytes.Length,
    ...
    private static void EnsureStreamAvailability(Stream stream) {
        Throws.When.Null(stream);

        if (!stream.CanRead) {
            throw new InvalidOperationException("Stream is not available for read.");
        }

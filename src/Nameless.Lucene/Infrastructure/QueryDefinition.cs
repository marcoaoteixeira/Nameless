...
public record QueryDefinition {
    ...
    ///     Defaults to <see cref="Constants.QUERY_MAXIMUM_RESULTS" /> if not
    ///     specified.
    /// </param>
    public QueryDefinition(Query query, Sort? sort = null, int start = 0, int limit = Constants.QUERY_MAXIMUM_RESULTS) {
        Query = query;
        Sort = sort ?? Sort.RELEVANCE;
        Start = Throws.When.LowerThan(start, compare: 0);
        Limit = Throws.When.LowerOrEqual(limit, compare: 0);
    }
}
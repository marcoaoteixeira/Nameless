...
public static class QueryableExtensions {
    ...
    extension<TItem>(IQueryable<TItem> self) {
        ...
        public IPage<TItem> ToPage(int start = 0, int limit = 10) {
            // We understand that could be a small chance that items being
            // added to the queryable during these two variables
            // assignment, making them differ but is a small trade-off.
            var items = self.Skip(start).Take(limit);
            var totalCount = self.Count();

            return new Page<TItem>(items, start, limit, totalCount);
        }
    }
}
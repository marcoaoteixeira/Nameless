...
public sealed class CircularBuffer<TElement> : ICircularBuffer<TElement> {
    ...
    public CircularBuffer(int capacity, TElement[] items) {
        ...
        Prevent.Argument.LowerThan(capacity, minValue: 1);
        Prevent.Argument.GreaterThan(paramValue: items.Length,
                                     paramName: nameof(items),
                                     maxValue: capacity,
                                     message: $"Too many items. Maximum number of items must be less or equal to {nameof(capacity)} => {capacity}");

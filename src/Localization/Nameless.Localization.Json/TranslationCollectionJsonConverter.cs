...
namespace Nameless.Localization.Json {
    ...
    public sealed class TranslationCollectionJsonConverter : JsonConverter {
        ...
        public static TranslationCollectionJsonConverter Default => new();

        #endregion

        #region Public Override Methods

        public override bool CanConvert(Type objectType) => typeof(TranslationCollection) == objectType ||
        ...
        public override object? ReadJson(JsonReader reader, Type objectType, object? existingValue, JsonSerializer serializer) {
            ...
            foreach (var translationCollection in translationCollections) {
                if (translationCollection == default) { continue; }

                var translationSet = new HashSet<TranslationEntry>();
                var translationJObjects = translationCollection.Values<JObject>().Where(_ => _ != default).ToArray();
                var translations = translationJObjects!.Values<JProperty>().ToArray();
                foreach (var translation in translations) {
                    if (translation == default) { continue; }

                    var valueJObjects = translation.Values<JToken>().Where(_ => _ != default).ToArray();
                    var values = valueJObjects!.Values<string>().ToArray();
                    translationSet.Add(new TranslationEntry(key: translation.Name, values: values!));
                }

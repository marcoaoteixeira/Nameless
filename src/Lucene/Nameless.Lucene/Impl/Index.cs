...
public sealed class Index : IIndex, IDisposable {
    ...
    public Index(Analyzer analyzer, string indexDirectoryPath, string name, ILogger<Index> logger) {
        _analyzer = Prevent.Argument.Null(analyzer, nameof(analyzer));
        _indexDirectoryPath = Prevent.Argument.NullOrWhiteSpace(indexDirectoryPath, nameof(indexDirectoryPath));
    ...
    private void InnerCommitChanges(IndexWriter writer) {
        try {
            ...
            LoggerHelper.IndexWriterOutOfMemory(_logger, ex);
            _indexWriter?.Dispose();
        } catch (Exception ex) {
            LoggerHelper.IndexWriterCommitFailure(_logger, ex);
        }
    }

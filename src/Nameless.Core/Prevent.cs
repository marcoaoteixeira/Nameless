...
public sealed class Prevent {
    ...
    private const string ARG_UNDEFINED_ENUM_MESSAGE =
        "Argument '{0}' provides value '{1}' that is not defined in enum '{2}'";
    ...
    public TValue Null<TValue>([NotNull] TValue? paramValue,
        ...
        throw exceptionCreator?.Invoke() ?? new ArgumentNullException(paramName,
    ...
    public TEnum UndefinedEnum<TEnum>(string paramValue,
        ...
        where TEnum : struct, Enum {
        if (!Enum.IsDefined(typeof(TEnum), paramValue)) {
            throw exceptionCreator?.Invoke() ?? new InvalidEnumArgumentException(string.IsNullOrWhiteSpace(message)

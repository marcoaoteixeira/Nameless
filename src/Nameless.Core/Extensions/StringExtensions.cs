...
public static class StringExtensions {
    ...
    extension(string self) {
        ...
        public MemoryStream ToMemoryStream(Encoding? encoding = null) {
            return new MemoryStream((encoding ?? CoreDefaults.Encoding).GetBytes(self));
        }
        ...
        public string Ellipsize(int characterCount, string ellipsis = "&#160;&#8230;", bool wordBoundary = false) {
        ...
        public byte[] ToHexByteArray() {
            var result = Enumerable.Range(start: 0, self.Length)
                                   .Where(index => index % 2 == 0)
                                   .Select(index => Convert.ToByte(self.Substring(index, length: 2),
        ...
        public string ReplaceAll(IDictionary<string, string> replacements) {
            var pattern = $"{string.Join(separator: "|", replacements.Keys)}";
        ...
        public string ToBase64(Encoding? encoding = null) {
            return Convert.ToBase64String((encoding ?? CoreDefaults.Encoding).GetBytes(self));
        }
        ...
        public string FromBase64(Encoding? encoding = null) {
            return (encoding ?? CoreDefaults.Encoding).GetString(Convert.FromBase64String(self));
        }
        ...
        public string Strip(params char[] chars) {
            ...
            return new string(result, startIndex: 0, cursor);
        }
        ...
        /// <returns>A stripped version of the <paramref name="self" /> parameter.</returns>
        public string Strip(Func<char, bool> predicate) {
            var result = new char[self.Length];
            var cursor = 0;
        ...
        public bool Any(params char[] chars) {
        ...
        public bool All(params char[] chars) {
        ...
        public string ToTechnicalName(int maxSize = 128) {
            ...
            while (result.Length > 0 && !result[index: 0].IsLetter()) {
                result = result[1..];
            }
        ...
        public string RemoveHtmlTags(bool htmlDecode = false) {
            ...
            var result = new string(content, startIndex: 0, cursor);
        ...
        public bool IsMatch(string regExp, RegexOptions regexOptions = RegexOptions.None) {
        ...
        public string Replace(string regExp, string replacement, RegexOptions regexOptions = RegexOptions.None) {
        ...
        public string[] Split(string regExp, RegexOptions regexOptions = RegexOptions.None) {
        ...
        public byte[] GetBytes(Encoding? encoding = null) {
            return (encoding ?? CoreDefaults.Encoding).GetBytes(self);
        }
        ...
        public string[] SplitUpperCase() {
            ...
            words.CopyTo(wordArray, index: 0);
        ...
        public string GetMD5(Encoding? encoding = null) {
            var buffer = (encoding ?? CoreDefaults.Encoding).GetBytes(self);
    ...
    extension(string self) {
        ...
        public string RemoveHead(string[] values, StringComparison comparison = StringComparison.OrdinalIgnoreCase) {
        ...
        public string RemoveTail(string[] values, StringComparison comparison = StringComparison.Ordinal) {
            foreach (var value in values) {
                if (string.IsNullOrWhiteSpace(value)) {
                    continue;
                }
    ...
    extension(string? self) {
        ...
        ///     string.
        /// </returns>
        public string WithFallback(string fallback) {
        ...
        /// </remarks>
        public string ToSnakeCase(char separator = '_') {
            if (self is null) { return string.Empty; }

            return string
                   .Concat(
                       self.Select((@char, index) => index > 0 &&
                                                     index < self.Length - 1 &&
                                                     char.IsUpper(@char) &&

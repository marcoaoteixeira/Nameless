...
public static class AssemblyExtensions {
    ...
    extension(Assembly self) {
        ...
        ///     Whether it should include the build metadata.
        ///     (e.g.: +build.NNN)
        /// </param>
        /// <returns>
        ///     A <see cref="string"/> representing the semantic version.
        /// </returns>
        public string GetSemanticVersion(string? prefix = null, string? suffix = null, bool includeBuildMetadata = false) {
            var version = self.GetName()
                              .Version ?? new Version(major: 0, minor: 0, build: 0);

            return version.ToSemanticVersion().ToString(prefix, suffix, includeBuildMetadata);
        }

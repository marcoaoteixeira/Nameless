...
public class MailkitMailingTests {
    private static MailkitMailing CreateSut(ISmtpClientFactory factory) {
        var logger = new LoggerMocker<MailkitMailing>()
            .WithAnyLogLevel()
            .Build();

        return new MailkitMailing(factory, logger);
    }

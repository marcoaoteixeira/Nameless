...
public class FileSystemMonitorTests {
    ...
    private readonly Mock<ILogger<FileMonitor>> _logger = new();

    private FileMonitorOptions DefaultOptions => new() {
        Filter = "*.txt",
    ...
    public async Task StartAsync_WhenTokenHasNoActiveCallbacks_ThrowsSmartFileSystemWatcherException() {
        ...
        // act & assert
        await Assert.ThrowsAsync<FileMonitorException>(
    ...
    public async Task StartAsync_WhenLockProbeExhausted_FiresOnErrorCallback() {
        ...
        var tcs = new TaskCompletionSource<FileMonitorErrorEventArgs>();

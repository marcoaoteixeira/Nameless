...
public class ZipCompressionServiceTests {
    ...
    private static ZipCompressionService CreateSut(ILogger<ZipCompressionService> logger = null) {
        logger ??= NullLogger<ZipCompressionService>.Instance;

        return new ZipCompressionService(logger);
    }

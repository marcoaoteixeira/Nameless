...
public sealed class LoggerMocker<T> : MockerBase<ILogger<T>> {
    ...
    public MockerBase<ILogger<T>>
        VerifyInformationCall(Func<string, bool> assertMessage = null, Times times = default) {
    ...
    public MockerBase<ILogger<T>> VerifyCallFor(Func<string, bool> assertMessage, LogLevel level = LogLevel.Debug,
                                                Times times = default) {

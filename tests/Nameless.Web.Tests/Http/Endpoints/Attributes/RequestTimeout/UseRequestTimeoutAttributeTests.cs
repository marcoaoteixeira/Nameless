...
[UnitTest]
public sealed class UseRequestTimeoutAttributeTests {
    [Fact]
    public void WhenCreatedWithPolicyName_ThenPolicyNameIsStored() {
        var attr = new UseRequestTimeoutAttribute("MyTimeoutPolicy");

        Assert.Equal("MyTimeoutPolicy", attr.PolicyName);
    }

    [Fact]
    public void WhenInspectingAttributeProperties_ThenAllowMultipleIsFalse() {
        var attr = typeof(UseRequestTimeoutAttribute);
